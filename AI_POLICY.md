...
homematicip-local-frontend. It is modeled on the
...
When reporting a bug, attach the **raw artifacts** — browser console output, debug logs, and
...
gates: `npm run lint`, `npm run type-check`, `npm test`, and `npm run build` (run all four at
once with `npm run validate` or `make validate`; CI runs the same steps on every pull request).
AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: homematicip-local-frontend is partly developed with AI assistance
