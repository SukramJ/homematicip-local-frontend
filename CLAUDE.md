...
26. **Do not write `window.isDirtyState`** (HA 2026.8+) — It looks like a global a custom panel could publish to, but it is an aggregate HA owns. `DirtyStateProviderMixin` recomputes it from `connectedDirtyStateProviders`, a Map module-local to HA's bundle that a separate bundle cannot join, and overwrites the global on every provider connect/update/disconnect. `dialog-box` is one such provider, so `showConfirmationDialog` — what `UnsavedGuard.promptDiscard` opens — clears anything we wrote at exactly the moment it would matter. Writing `false` is worse still: it would clear a genuinely dirty HA provider. `UnsavedGuard` stays self-contained until HA exposes a registration API.
